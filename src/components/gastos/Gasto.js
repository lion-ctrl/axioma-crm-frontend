...
const Gasto = ({ gasto }) => {
	// context
	const { eliminarGasto } = useContext(gastoContext);

	const eliminarGastoClick = (_id) => {
		Swal.fire({
			title: "¿Seguro?",
	...
	return (
		<div className="mt-4 bg-white rounded p-6 md:grid md:grid-cols-2 md:gap-4 shadow-lg">
			<div>
				<p className="font-bold text-gray-800">{gasto.nombre}</p>
				<p className="my-2">Tipo de Gasto: {gasto.tipo}</p>
				<p className="my-2">Gasto: {gasto.costo}$</p>
			</div>
			<div>
				<Link
					to={`/gasto/${gasto._id}`}
				...
				<button
					onClick={() => eliminarGastoClick(gasto._id)}
